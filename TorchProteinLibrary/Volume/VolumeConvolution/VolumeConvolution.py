...
class VolumeConvolutionFunction(Function):
	...
	def forward(ctx, volume1, volume2):
		...
		if len(volume1.size())==4:
			...
			output = torch.FloatTensor(batch_size, volume_size, volume_size, volume_size).cuda()
		else:
			raise ValueError('VolumeConvolutionFunction: ', 'Incorrect input size:', volume1.size()) 
		
		output.fill_(0.0)
		cppVolumeConvolution.VolumeConvolution_forward( volume1, volume2, output)
	...
	def backward(ctx, grad_output):
		...
		if len(grad_output.size()) == 4:
			...
			grad_input_volume1 = torch.FloatTensor(batch_size, volume_size, volume_size, volume_size).cuda().fill_(0.0)
			grad_input_volume2 = torch.FloatTensor(batch_size, volume_size, volume_size, volume_size).cuda().fill_(0.0)
		else:
			raise ValueError('VolumeConvolutionFunction: ', 'Incorrect input size:', grad_output.size()) 
		
		# print torch.sum(grad_output.data)
		
		cppVolumeConvolution.VolumeConvolution_backward(grad_output.data, grad_input_volume1, grad_input_volume2, volume1, volume2)
		grad_input_volume2 = flip(flip(flip(grad_input_volume2, dim=1),dim=2),dim=3)
		# print torch.sum(grad_input_volume1)
		# print torch.sum(grad_input_volume2)
...
class VolumeConvolution(Module):
	def __init__(self, num_features):
		...
		self.W = torch.nn.Parameter(torch.FloatTensor(1, num_features, 1, 1, 1).normal_().cuda())
	...
	def forward(self, input_volume1, input_volume2):
		...
		# volume1 = input_volume1.resize(batch_size*num_features, volume_size, volume_size, volume_size).contiguous()
		# volume2 = input_volume2.resize(batch_size*num_features, volume_size, volume_size, volume_size).contiguous()
		...
		# output = output.resize(batch_size, num_features, volume_size, volume_size, volume_size).contiguous()
