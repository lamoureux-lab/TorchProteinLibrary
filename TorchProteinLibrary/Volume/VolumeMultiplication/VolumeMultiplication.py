...
class VolumeCrossMultiply(nn.Module):
    ...
    def forward(self, volume1, volume2, R, T):
        ...
        TransMat = torch.stack([(RotMat[:, 0, :]*T).sum(dim=1), (RotMat[:, 1, :] *
                                                                 T).sum(dim=1), (RotMat[:, 2, :]*T).sum(dim=1)], dim=1)
        # TransMat = T
        ...
        for i in range(1, num_features):
            volume1_unpacked.append(volume1[:, i:num_features, :, :, :])
            volume2_unpacked.append(volume2[:, 0:num_features-i, :, :, :])
