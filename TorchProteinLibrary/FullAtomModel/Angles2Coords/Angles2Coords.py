...
class Angles2CoordsFunction(Function):
	...
	# @profile	
	@staticmethod
	def forward(ctx, input_angles_cpu, sequenceTensor, num_atoms, polymer_type, na_num_atoms, chain_names):
	...
	def backward(ctx, grad_atoms_cpu, *kwargs, polymer_type=1, chain_names=torch.zeros(240, 1, dtype=torch.uint8).fill_(ord('A'))):
		...
		_FullAtomModel.Angles2Coords_backward(grad_atoms_cpu, grad_angles_cpu, sequenceTensor, input_angles_cpu, polymer_type, chain_names)
...
class Angles2Coords(Module):
	...
	def forward(self, input_angles_cpu, sequences, polymer_type, na_num_atoms, chain_names):
		...
		return Angles2CoordsFunction.apply(input_angles_cpu, stringListTensor, num_atoms, self.polymer_type, na_num_atoms, chain_names)
