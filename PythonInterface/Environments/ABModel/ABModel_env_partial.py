...
class ABModelEnv(gym.Env):
	...
	def compute_energy(self, angles):
		...
		V2 = torch.min(torch.FloatTensor([10.0]).cuda(), torch.sum(torch.sum(r12,1),1))
	...
	def _step(self, action):
		...
		reward = self.prev_energy - trial_energy
		self.angles.copy_(trial_angles)
		self.prev_energy = trial_energy
		self.coords.copy_(self.new_coords)
		return new_distributions, reward[0], done, {}
		
		# else:
		# 	reward = 0.0
		# 	old_distributions = self.compute_internal_variables(self.angles, self.coords)
			
		# 	return old_distributions, reward, done, {}



	def _reset(self):
		self.angles.copy_(0.01*self.action_space.sample())
