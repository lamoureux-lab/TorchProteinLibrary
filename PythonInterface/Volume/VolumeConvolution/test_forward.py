...
def fill_exp(r0, volume):
	...
	for x in xrange(volume_size):
		for y in xrange(volume_size):
			for z in xrange(volume_size):
				r = np.array([x,y,z]) - r0
...
if __name__=='__main__':
	...
	vc.W.data.fill_(1.0)
	v_size = 30
	inp1 = Variable(torch.FloatTensor(1, 1, v_size, v_size, v_size).fill_(0.0).cuda())
	inp2 = Variable(torch.FloatTensor(1, 1, v_size, v_size, v_size).fill_(0.0).cuda())
