...
class TypedCoords2VolumeFunction(Function):
	...
	def forward(ctx, input_coords_gpu, num_atoms_of_type_gpu, offsets_gpu, box_size=120):
		...
		if len(input_coords_gpu.size())==2:
			...
			volume_gpu = torch.zeros(batch_size, num_atom_types, box_size, box_size, box_size, dtype=torch.float, device='cuda')
		else:
	...
	def backward(ctx, grad_volume_gpu):
		...
		input_coords_gpu, num_atoms_of_type_gpu, offsets_gpu = ctx.saved_tensors
		
		if len(grad_volume_gpu.size()) == 5:
			num_coords = input_coords_gpu.size(1)
			batch_size = grad_volume_gpu.size(0)
			grad_coords_gpu = torch.zeros(batch_size, num_coords, dtype=torch.double, device='cuda')
		else:
