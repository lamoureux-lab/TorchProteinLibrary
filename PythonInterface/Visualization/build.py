...
sources = [ os.path.join(lib_dir,'Visualization/visualization_interface.cpp'),
			os.path.join(lib_dir,'VolumeVisualization/vol_viz_interface.cpp')
            ]
headers = [	os.path.join(lib_dir,'Visualization/visualization_interface.h'),
			os.path.join(lib_dir,'VolumeVisualization/vol_viz_interface.h')
			]
...
ffi = create_extension(
	...
	extra_link_args = [],
	libraries = ["FULL_ATOM_MODEL","MARCHINGCUBES", "TH_MATH", "VISUALIZATION", "GLFRAMEWORK", "GL", "GLU", "glut"],
