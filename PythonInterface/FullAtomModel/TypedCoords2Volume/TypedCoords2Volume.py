...
class TypedCoords2VolumeFunction(Function):
	...
	def backward(self, grad_volume_gpu):
		...
		elif len(grad_volume_gpu.size()) == 4:
			raise(Exception('Not implemented'))

		else:
			raise ValueError('TypedCoords2VolumeFunction: ', 'Incorrect input size:', input_angles_cpu.size()) 
		
		cppTypedCoords2Volume.TypedCoords2Volume_backward(grad_volume_gpu, grad_coords_gpu, self.input_coords_gpu, self.num_atoms_of_type, self.offsets)
