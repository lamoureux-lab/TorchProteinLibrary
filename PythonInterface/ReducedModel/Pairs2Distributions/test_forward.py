...
def test_foward_nonvectorized():
	...
	sigma = 2.0
	distr_man = np.zeros((num_atoms, num_types, num_types, num_bins))
	distr = distr.resize_(num_atoms, num_types, num_types, num_bins)
	for i in range(num_atoms):
		...
		for j in range(num_atoms):
			...
			for k in range(num_bins):
				r = k*resolution
				distr_man[i,atype1, atype2, k] += np.exp(-(r-dist)*(r-dist)/sigma)
	
	f, axarr = plt.subplots(2)
	axarr[0].imshow(distr_man.reshape((num_atoms,num_types*num_types*num_bins)))
	distr_np = torch.FloatTensor(num_atoms,num_types,num_types,num_bins).copy_(distr).numpy()
	axarr[1].imshow(distr_np.reshape((num_atoms,num_types*num_types*num_bins)))
