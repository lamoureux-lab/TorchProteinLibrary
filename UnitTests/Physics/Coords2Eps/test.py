...
from TorchProteinLibrary.Physics import AtomNames2Params, ElectrostaticParameters, Coords2Eps
import _Volume
...
class TestCoords2Eps(TestAtomNames2Params):
	...
	def setUp(self):
		...
		box_size = 60
		res = 0.5
		self.c2e = Coords2Eps(box_size=box_size, resolution=res)
...
class TestCoords2Eps_forward(TestCoords2Eps):
	...
	def runTest(self):
		...
		volume = self.c2e(	ce_coords.to(device=self.device, dtype=self.dtype), 
							params.to(device=self.device, dtype=self.dtype), 
							num_atoms.to(device=self.device))
		
		# _Volume.Volume2Xplor(volume[0,:,:,:].to(device='cpu'), "tmp.xplor", 1.0)

		import pyvista as pv
		p = pv.Plotter(point_smoothing=True)
		vol = volume[0,:,:,:].to(device='cpu').numpy()
		print(vol.shape)
		p.add_volume(vol, cmap="coolwarm", opacity="linear_r")
		p.show()
