...
def test_gradient():
	L=10
	x0 = torch.zeros(1, 3, L, dtype=torch.float, device='cuda').normal_().requires_grad_()
	x1 = torch.zeros(1, 3, L, dtype=torch.float, device='cuda').normal_()
...
def test_gradient_batch():
	L=10
	batch_size = 2
	x0 = torch.zeros(batch_size, 3, L, dtype=torch.float, device='cuda').normal_().requires_grad_()
	x1 = torch.zeros(batch_size, 3, L, dtype=torch.float, device='cuda').normal_()
	...
	for b in range(0,batch_size):
		...
		# plt.plot(back_grad_x0[b,1,:].numpy(),'-bo', label = 'an beta')
		plt.plot(back_grad_x0[b,2,:].numpy(),'.-b', label = 'an omega')
		plt.legend()
