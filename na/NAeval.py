...
# log3 optimizer(Adam) and learning rate
    ...
    write_pdbs_1(pdb_ids, coords_aft1000, chainnames, resnames, resnums, atomnames, num_atoms, 1000, loss_at1000)
    ...
    write_pdbs_1(pdb_ids, coords_aft2000, chainnames, resnames, resnums, atomnames, num_atoms, 2000, loss_at2000)
    ...
    write_pdbs_1(pdb_ids, coords_aft4000, chainnames, resnames, resnums, atomnames, num_atoms, 4000, loss_at4000)
    # save pdb and loss per x epochs then repeat as many times as desired
...
def loss_per_epoch_plot(loss,epochs, pdb_ids):
    batch_size = pdb_ids.size(0)
    for batch_idx in range(batch_size):
        fig, ax = plt.subplots()
        ax.plot(epochs[batch_idx], loss[batch_idx])
        # ax.set_ylim([0, 4])
        ax.set_xlabel("epochs", fontsize=12)
        ax.set_ylabel("rmsd (A)", fontsize=12)

        plt.savefig('/u2/home_u2/fam95/Downloads/RNA/Batch4/' + pdb_ids[batch_idx] + '/' + pdb_ids[batch_idx] + '_4e3.png')
...
def write_pdbs(pdb_ids, coords_src, chainnames, resnames, resnums, atomnames, num_atoms):
    batch_size = pdb_ids.size(0)
    for batch_idx in range(batch_size):
        new_file = '/u2/home_u2/fam95/Downloads/RNA/Batch4/' + pdb_ids[batch_idx] + '/' + pdb_ids[batch_idx] + ".pdb"
        FullAtomModel.writePDB(new_file, coords_src[batch_idx], chainnames[batch_idx], resnames[batch_idx], resnums[batch_idx], atomnames[batch_idx], num_atoms[batch_idx])

def write_pdb(pdb_id, coords_src, chainnames, resnames, resnums, atomnames, num_atoms, epoch, loss):
    new_file = '/u2/home_u2/fam95/Downloads/RNA/Batch4/' + pdb_id + '/' + pdb_id + '_' + epoch + '_' + loss + ".pdb"
...
def write_pdbs_1(pdb_ids, coords_src, chainnames, resnames, resnums, atomnames, num_atoms,epoch, loss_list):
    batch_size = pdb_ids.size(0)
    for batch_idx in range(batch_size):
        new_file = '/u2/home_u2/fam95/Downloads/RNA/Batch4/' + pdb_ids[batch_idx] + '/' + pdb_ids[batch_idx] + epoch + '_' + loss_list[batch_idx] +".pdb"
        FullAtomModel.writePDB(new_file, coords_src[batch_idx], chainnames[batch_idx], resnames[batch_idx], resnums[batch_idx], atomnames[batch_idx], num_atoms[batch_idx])
if __name__ == "__main__":
    ...
    #read csv and load into df
    csv = '/u2/home_u2/fam95/Downloads/RNA/Batch4/na_pdb_dataset_long_chains_shorttest.csv'
    ...
    #get only chains associated with chain ids
    polymer_type = 2
    coords_dst, chainnames, resnames, resnums, atomnames, mask, num_atoms, poly_types = parse_pdbs(file_paths, chain_ids, polymer_type)
    ...
    loss_1000_df = pd.DataFrame.from_dict(loss_at1000, 'index', columns=['pdb_id', 'loss at 1000'])
    print(loss_1000_df)
    ...
    loss_2000_df = pd.DataFrame.from_dict(loss_at2000, 'index', columns=['pdb_id', 'loss at 2000'])
    print(loss_2000_df)
    merged_df_2000 = pd.merge(merged_df_1000, loss_2000_df, how='inner', on='pdb_id')

    loss_4000_df = pd.DataFrame.from_dict(loss_at4000, 'index', columns=['pdb_id', 'loss at 4000'])
    print(loss_4000_df)
